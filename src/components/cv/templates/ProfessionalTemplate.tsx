...
export function ProfessionalTemplate({ cvData }: { cvData: CVData }) {
  ...
  return (
    <div className="p-10 font-body text-[10px] leading-snug bg-white text-foreground h-full overflow-hidden">
      {/* Header */}
              ...
              <div className="space-y-4">
                {experience.map(exp => (
                  <div key={exp.id}>
                    ...
                    <h4 className="font-semibold text-accent">{exp.company || "Company"}</h4>
                ...
                {education.map(edu => (
                  <div key={edu.id}>
                    <div className="flex justify-between items-baseline">
                      ...
                      <p className="text-muted-foreground text-[9px]">{edu.startDate} - {edu.endDate}</p>
                    </div>
                    <h4 className="font-semibold text-accent">{edu.institution || "Institution"}</h4>
